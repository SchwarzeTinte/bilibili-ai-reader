...
def download_audio(part: dict[str, Any], auth_source: str | None = None) -> Path:
    ...
    try:
        ...
        raise BilibiliReaderError(f"音频下载失败：{exc}") from exc
    expected = target_dir / f"{base_name}.mp3"
    if expected.exists():
        return expected
    candidates = sorted(target_dir.glob(f"{base_name}.*"), key=lambda path: path.stat().st_mtime, reverse=True)
    if not candidates:
        raise BilibiliReaderError("下载命令已结束，但没有找到生成的音频文件。")
    return candidates[0]
